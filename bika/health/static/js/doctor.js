        ...
        $("input[id*=DoctorID]").combogrid({
            ...
            select: function( event, ui ) {
                ...
                if($(".portaltype-batch").length > 0 && $(".template-base_edit").length > 0) {
                    $(".jsDoctorTitle").remove();
                    $("#archetypes-fieldname-DoctorID").append("<span class='jsDoctorTitle'><a class='edit_doctor' href='"+window.portal_url+"/doctors/"+ui.item.DoctorSysID+"/edit'>"+ui.item.Title+"</span>");
                    $('a.edit_doctor').prepOverlay(editdoctor_overlay); 
                }
                return false;
            }
